...
router.post('/services', verifyAdmin, async (req, res) => {
  try {
    ...
    const { name, description, price, duration, isActive: is_active = true } = req.body;
    ...
    // Create service - the slug will be auto-generated by the beforeCreate hook
    const serviceData = {
      name,
    ...
    serviceJson.price = Number(serviceJson.price);
    
    console.log('Service created successfully:', serviceJson);
    res.status(201).json(serviceJson);
  } catch (error) {
...
router.put('/services/:id', verifyAdmin, async (req, res) => {
  try {
    ...
    // Handle price conversion if it exists in the update
    if (price !== undefined) {
    ...
    // Convert to plain object and ensure price is a number
    const updatedService = service.toJSON();
